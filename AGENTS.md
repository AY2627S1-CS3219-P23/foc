<!--
  ...
  declared in the root .mcp.json (developer tooling only - no product
...
**`postgres` setup.** The connection string in `.mcp.json` is built from
environment variables and points at `notification-db` on its published
host port. Bring the DB up (`docker compose up -d notification-db`) and
export the variables from your `.env` before starting Claude Code:

```sh
set -a; source .env; set +a
```

`.env` is git-ignored, so no credentials are committed — the file only
holds `${NOTIFICATION_DB_USER}`-style references. If the variables are
not exported, this one server fails to start and the rest keep working.
It currently targets the notification DB only; point it at another
service's database by editing the connection string locally, or add a
second entry when that service exists.

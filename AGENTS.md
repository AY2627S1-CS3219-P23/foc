...
| `postgres` | Inspecting a service-owned database (schema generated by JPA, stored rows) without installing `psql` | Needs a running DB + one env var, see below |
...
**`postgres` setup.** The entry in `.mcp.json` names no database — we
run database-per-service, so there is no single "the" DB, and nothing
service-specific is committed. The server picks up its default from the
`POSTGRES_CONNECTION_STRING` environment variable, which each person
sets in their own git-ignored `.env` to whichever service DB they work
on:

```sh
# in .env
POSTGRES_CONNECTION_STRING=postgresql://notification:<password>@localhost:5433/notification
```

Export it before starting Claude Code (`set -a; source .env; set +a`).
Leaving it unset is fine: the server still starts, and only errors if a
tool is called without a connection string.

**Targeting a different database.** Every tool the server exposes except
`pg_copy_between_databases` takes an optional `connectionString`
argument that overrides `POSTGRES_CONNECTION_STRING` for that one call, so a
single server entry covers every service DB. Do not add a second server
entry per service. One caveat: issue calls against different databases
one at a time — two calls fired concurrently can end up sharing a
connection, so the second may read the first one's database.
...
in the row above. Agents should take the port from this table rather

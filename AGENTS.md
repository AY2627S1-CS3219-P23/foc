...
| `postgres` | Inspecting a service-owned database (schema generated by JPA, stored rows) without installing `psql` | Needs env vars + a running DB, see below |
...
**`postgres` setup.** `POSTGRES_CONNECTION_STRING` in `.mcp.json` is
built from environment variables and is only the **default** database —
it points at `notification-db`, the one service DB that exists today.
Bring it up (`docker compose up -d notification-db`) and export the
variables from your `.env` before starting Claude Code:

```sh
set -a; source .env; set +a
```

`.env` is git-ignored, so no credentials are committed — `.mcp.json`
only holds `${NOTIFICATION_DB_USER}`-style references. If the variables
are not exported, this one server fails to start and the rest keep
working.

**Targeting another service's database.** We run database-per-service,
so there will eventually be one DB per backend service. A single
`postgres` server entry covers all of them: every tool it exposes except
`pg_copy_between_databases` takes an optional `connectionString`
argument that overrides the default for that one call. There is no need
to add a second server entry per service.

| Service DB | Host port | Status |
| --- | --- | --- |
| `notification-db` | `${NOTIFICATION_DB_HOST_PORT:-5433}` | available (the default) |
...
header already records), publishes it on a free host port, adds the
`*_DB_HOST_PORT` variable to `.env.example`, and fills in the row above.
Pick a port that no other service has claimed in this table. Agents
should read the port from this table and pass the matching
`connectionString` rather than assuming the default.
